...
import java.net.URI;
public class TopNWordsMapper extends Mapper<Object, Text, WordFrequencyWritable, IntWritable> {
    ...
    public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        ...
        for (String token : tokens) {
            ...
            if (!stopWords.contains(lowerCaseToken) && lowerCaseToken.length() > 6) {
                WordFrequencyWritable wordFrequency = new WordFrequencyWritable(new Text(lowerCaseToken), new IntWritable(1));
                context.write(wordFrequency, one);
            }
        }
    }
}
