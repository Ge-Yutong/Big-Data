...
class LogProcessor:
    def __init__(self, topic, file, num_partitions):
        self.topic = topic
        self.file = file
        self.num_eof_received = 0
    ...
    def produce_data_to_kafka(self):
        ...
        # Send 'EOF' message to each partition
        for i in range(self.num_partitions):
            producer.produce(self.topic, 'EOF', partition=i)
        producer.flush()
    ...
    def parse_and_process_batch(self, df, epoch_id):
        ...
        if split_df.where(col('value_str') == 'EOF').count() > 0:
            self.num_eof_received += 1
            if self.num_eof_received == self.num_partitions:
                self.EOF_RECEIVED = True

        if not self.EOF_RECEIVED:
            split_df = split_df.select(
                regexp_extract(col('value_str'), PATTERN, 1).alias('host'),
                regexp_extract(col('value_str'), PATTERN, 4).alias('date_time'),
                regexp_extract(col('value_str'), PATTERN, 5).alias('method'),
                regexp_extract(col('value_str'), PATTERN, 6).alias('endpoint'),
                regexp_extract(col('value_str'), PATTERN, 7).alias('protocol'),
...
def main():
    ...
    processor = LogProcessor(kafka_topic, log_file, num_partitions)
